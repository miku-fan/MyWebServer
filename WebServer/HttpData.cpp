...
void HttpData::handleRead() {
  ...
  cout<<"before rd events is "<<events_<<endl;
  do {
    bool zero = false;
  ...
  cout<<"after rd events is "<<events_<<endl;
}
...
AnalysisState HttpData::analysisRequest() {
  if (method_ == METHOD_POST) {
    ...
    if (fileName_ == "favicon.ico") {
      ...
      header += "Server: MIKUFAN's Web Server\r\n";
...
void HttpData::handleError(int fd, int err_num, string short_msg) {
  ...
  body_buff += "<hr><em> MIKUFAN's Web Server</em>\n</body></html>";

  header_buff += "HTTP/1.1 " + to_string(err_num) + short_msg + "\r\n";
  header_buff += "Content-Type: text/html\r\n";
  header_buff += "Connection: Close\r\n";
  header_buff += "Content-Length: " + to_string(body_buff.size()) + "\r\n";
  header_buff += "Server: MIKUFAN's Web Server\r\n";
  ;
  header_buff += "\r\n";
  // 错误处理不考虑writen不完的情况
...
void HttpData::handleClose() {
  ...
  std::cout<<"in HandleClose"<<std::endl;
  shared_ptr<HttpData> guard(shared_from_this());
  cout<< "shared_ptr count is " <<guard.use_count() <<endl;
